...
const MultipleChoice = ({ setOpen, update = false, quizId, questionId }) => {
  ...
  const selectedQuestion = questionId
    ? quiz.questions.find((q) => q._id === questionId)
    : null;
  ...
  const handleSubmit = (e) => {
    ...
    for (let i = 0; i < correctObj.length; i++) {
      setMultipleChoiceAnswer({
        ...
        correctAnswer: multipleChoiceAnswer.correctAnswer.push(
          correctObj[i].answer
        ),
      });
    }
    ...
    for (let i = 0; i < incorrectObj.length; i++) {
      setMultipleChoiceAnswer({
        ...multipleChoiceAnswer,
        incorrectAnswer: multipleChoiceAnswer.incorrectAnswer.push(
          incorrectObj[i].answer
        ),
      });
    }
    if (!update) {
      ...
      setOpen(false);
      dispatch(updateQuestion(quizId, questionId, multipleChoiceAnswer));
    }
  ...
  useEffect(() => {
    if (selectedQuestion) {
      const { question, correctAnswer, incorrectAnswer } = selectedQuestion;
      setAnswer(correctAnswer.length + incorrectAnswer.length)
      setMultipleChoiceAnswer({ ...multipleChoiceAnswer, question, incorrectAnswer, correctAnswer });
    }
  }, [update]);
  ...
  return (
    <MCBox>
      <Form onSubmit={handleSubmit}>
        ...
        >
          <TextField
            value={multipleChoiceAnswer.question && multipleChoiceAnswer.question}
            required
            id="outlined-basic"
            label="Question"
            variant="outlined"
          ...
          <Grid container spacing={2} justify="center">
            {[...Array(answer)].map((ans, id) => {
              // let answers = multipleChoiceAnswer.correctAnswer.concat(multipleChoiceAnswer.incorrectAnswer)
              // console.log(answers)
              return (
                <Grid item key={id} xs={12} sm={6}>
                  <TextField
                    //value={answer && answers[id]}
                      ...
                      startAdornment: (
                        <InputAdornment position="start">
                          <IconButton aria-label="add to favorites">
                            <Checkbox
                              onChange={(e) =>
                                markCorrect(id, e.target.checked)
                              }
